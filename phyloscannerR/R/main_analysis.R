...
initialise.phyloscanner <- function(
  ...
  if(verbosity!=0){
    cat("Checking for window coordinates in tree identifiers...")
  }
...
phyloscanner.analyse.trees <- function(
  ...
  if(length(hosts)>1){
    ...
    ptrees <- sapply(ptrees, function(ptree) {
      ...
      ptree$classification.results <- classify(ptree, allow.mt, n.mt, p.mt, identify.multifurcation, relaxed.ancestry,verbosity==2, no.progress.bars)
...
phyloscanner.analyse.tree <- function(
  ...
  phyloscanner.analyse.trees(
    tree.file.directory = tree.file.directory, 
    tree.file.regex = basename(tree.file.name), 
    splits.rule = splits.rule, 
    sankoff.k = sankoff.k, 
    sankoff.unassigned.switch.threshold = sankoff.unassigned.switch.threshold,
    continuation.unassigned.proximity.cost = continuation.unassigned.proximity.cost, 
    outgroup.name = outgroup.name, 
    multifurcation.threshold = multifurcation.threshold, 
    guess.multifurcation.threshold = guess.multifurcation.threshold,
    user.blacklist.directory = user.blacklist.directory, 
    user.blacklist.file.regex = basename(user.blacklist.file.name), 
    duplicate.file.directory = duplicate.file.directory,
    duplicate.file.regex = basename(duplicate.file.name), 
    ...
    n.mt = n.mt,
    p.mt = p.mt,
    identify.multifurcation = identify.multifurcation,
    ...
    use.ff = use.ff, 
    prune.blacklist = prune.blacklist, 
    count.reads.in.parsimony = count.reads.in.parsimony, 
    verbosity = verbosity, 
...
rename.user.blacklist.tips <- function(ptree) {
  ...
  if(!is.null(ptree$blacklist)){
    ...
    if(length(ptree$blacklist)>0){
      ptree$tree      <- ptree$tree %>% rename.blacklisted.tips(ptree$blacklist, "USER")
    }
  }
  ptree
}
