...
lgamma_prod_vb <- function(llbd, a, b){
  # deal with small lambda whose exp is 0
  c <- min((max(llbd + log(b)) - min(llbd + log(b)))/2,700)
  ans <- sum( a * log(b) ) - sum( lgamma(a) ) +
...
{        
  ...
  for (i in 1:nrow(mc$dlu))
  {
    tmp <- mc$dlu[UPDATE_ID==i,c('SAMPLING_CATEGORY','WHO')]
    dprior2[[i]] <- dprior[J(tmp),nomatch=0L]
    # dprior2[[i]] <- dprior[SAMPLING_CATEGORY==tmp$SAMPLING_CATEGORY & WHO==as.character(tmp$WHO),]
  }
  ...
  for(i in 1L:mc$n)
  {
    ...
    if(mc$with.sampling)
    {
      ...
      LOG_LAMBDA.prop[update.pairs] <- mapply( function(x,y){ rgamss(1,shape=x,scale=y) }, mc$dlt$TRM_OBS[update.pairs] + mc$pars$ALPHA[update.pairs], 1/(S.prop[update.pairs] + BETA.prop))
      ...
      log.prop.ratio <- lgamma_prod_vb(LOG_LAMBDA.curr[update.pairs],
                                       ...
                                       BETA.curr + S.curr[update.pairs])-
        lgamma_prod_vb(LOG_LAMBDA.prop[update.pairs],
                       mc$dlt$TRM_OBS[update.pairs] + mc$pars$ALPHA[update.pairs],
                       BETA.prop + S.prop[update.pairs])
