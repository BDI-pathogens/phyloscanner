...
source.attribution.mcmc.getKeyQuantities<- function(infile=NULL, mc=NULL, pars=NULL, dobs=NULL, control=NULL)
{
    if (is.null(mc) & is.null(pars) & !is.null(infile)) {
        ...
        if (tmp == "mc" & length(infile) > 1) {
            ...
            if (is.null(mc) & is.null(pars) & !is.null(infile) &
                length(infile) > 1 & all(grepl("rda$", infile)) &
                all(unlist(lapply(infile, function(x) {
                  load(x) == "mc"
                })))) {
            ...
            cat("\nTotal number of MCMC iterations loaded from file ",
                nrow(mc$pars$LOG_LAMBDA))
    ...
    if (!is.null(mc)) {
        ...
        if (is.na(thin))
            thin <- 1
        if (burnin.n > 0) {
            cat("\nRemoving burnin in set to ", 100 * burnin.p,
                "% of chain, total iterations=", burnin.n)
        ...
        if (thin > 1) {
            ...
            mc$pars$LOG_LAMBDA <- pars[mc$pars$LOG_LAMBDA, ,
                drop = FALSE]
        }
        ...
        pars[, `:=`(VARIABLE, pars[, gsub("([A-Z_]+)-([0-9]+)",
            "\\1", variable)])]
        pars[, `:=`(TRM_CAT_PAIR_ID, pars[, as.integer(gsub("([A-Z_]+)-([0-9]+)",
            "\\2", variable))])]
        ...
        setnames(pars, c("TR_TRM_CATEGORY", "REC_TRM_CATEGORY"),
            c("TR_TARGETCAT", "REC_TARGETCAT"))
        tmp1 <- pars[, {
            tmp <- min((max(value) - min(value))/2, 700)
            list(VALUE = exp(-tmp) * sum(exp(tmp + value)))
        }, by = c("VARIABLE", "TR_TARGETCAT", "REC_TARGETCAT",
            "SAMPLE")]
        tmp2 <- pars[, {
            tmp <- min((max(value) - min(value))/2, 700)
    ...
    if (any(is.na(pars$VALUE))) {
        cat("\nRemoving NA output for samples n=", nrow(subset(pars,
            is.na(VALUE))))
        pars <- subset(pars, !is.na(VALUE))
    }
    cat("\nComputing flows...")
    z <- pars[, list(P = names(control$quantiles), Q = unname(quantile(VALUE,
        p = control$quantiles))), by = c("TR_TARGETCAT", "REC_TARGETCAT")]
    z <- dcast.data.table(z, TR_TARGETCAT + REC_TARGETCAT ~ P,
        value.var = "Q")
    z[, `:=`(LABEL, paste0(round(M * 100, d = 1), "%\n[", round(CL *
        100, d = 1), "% - ", round(CU * 100, d = 1), "%]"))]
    z[, `:=`(LABEL2, paste0(round(M * 100, d = 1), "% (", round(CL *
        100, d = 1), "%-", round(CU * 100, d = 1), "%)"))]
    ...
    gc()
    cat("\nComputing WAIFM...")
    z <- pars[, list(REC_TARGETCAT = REC_TARGETCAT, VALUE = VALUE/sum(VALUE)),
        by = c("TR_TARGETCAT", "SAMPLE")]
    z <- z[, list(P = names(control$quantiles), Q = unname(quantile(VALUE,
        p = control$quantiles))), by = c("TR_TARGETCAT", "REC_TARGETCAT")]
    z <- dcast.data.table(z, TR_TARGETCAT + REC_TARGETCAT ~ P,
        value.var = "Q")
    z[, `:=`(LABEL, paste0(round(M * 100, d = 1), "%\n[", round(CL *
    ...
    gc()
    cat("\nComputing sources...")
    z <- pars[, list(TR_TARGETCAT = TR_TARGETCAT, VALUE = VALUE/sum(VALUE)),
        by = c("REC_TARGETCAT", "SAMPLE")]
    z <- z[, list(P = names(control$quantiles), Q = unname(quantile(VALUE,
        p = control$quantiles))), by = c("TR_TARGETCAT", "REC_TARGETCAT")]
    z <- dcast.data.table(z, TR_TARGETCAT + REC_TARGETCAT ~ P,
        value.var = "Q")
    ...
    if (length(control$flowratios) > 0) {
        ...
        flowratio.flows <- unique(c(sapply(control$flowratios,
            "[[", 2), sapply(control$flowratios, "[[", 3)))
        if (!all(flowratio.flows %in% colnames(z))) {
            stop("Cannot find all requested flow ratios in MCMC output with column names \"",
                paste(colnames(z), collapse = "\", \""), "\"")
        }
        set(z, NULL, colnames(z)[!colnames(z) %in% c("SAMPLE",
            unlist(control$flowratios))], NULL)
        for (ii in seq_along(control$flowratios)) {
            if (!control$flowratios[[ii]][2] %in% colnames(z)) {
                cat("\nColumn names in MCMC output are", colnames(z))
                warning("\nColumn name ", control$flowratios[[ii]][2],
                  " not in MCMC output. Setting to 0.")
                set(z, NULL, control$flowratios[[ii]][2], 0)
            }
            if (!control$flowratios[[ii]][3] %in% colnames(z)) {
                cat("\nColumn names in MCMC output are", colnames(z))
                warning("\nColumn name ", control$flowratios[[ii]][3],
                  " not in MCMC output. Setting to 0.")
        ...
        z <- melt(z, id.vars = "SAMPLE", value.name = "VALUE",
            variable.name = "FLOWRATIO_CAT")
        z <- z[, list(P = names(control$quantiles), Q = unname(quantile(VALUE,
            p = control$quantiles))), by = c("FLOWRATIO_CAT")]
        z <- dcast.data.table(z, FLOWRATIO_CAT ~ P, value.var = "Q")
        z[, `:=`(LABEL, paste0(round(M, d = 2), "\n[", round(CL,
            d = 2), " - ", round(CU, d = 2), "]"))]
        z[, `:=`(LABEL2, paste0(round(M, d = 2), " (", round(CL,
            d = 2), "-", round(CU, d = 2), ")"))]
        z[, `:=`(STAT, "flow_ratio")]
