...
group $i$, we determine all components of $\xi$ that involve $s_i$. In our example, for sources where
$i=1$, the components of $\xi$ to update are $(\xi_{11},\xi_{12})$; and
for recipients where $i=2$, the components of $\xi$ to update are
$(\xi_{12},\xi_{22})$. An MCMC sweep counts the number of MCMC iterations needed
...
* `Number of iterations: 1000`: The total number of iterations is set to the
