---
title: "Estimating transmission flows under heterogeneous sampling - a first example"
date: "2019-04-30"
...
* $p(\lambda_{ij})$ an uninformative prior distribution. We use a Gamma distribution with
parameters $\alpha_i=0.8/4$ and $\beta=0.8/Z$ with $Z= \sum_{ij | n_{ij}>0} n_{ij}/(s_i*s_j) + \sum_{ij | n_{ij}>0}
(1-s_i*s_j)/(s_i*s_j)$. This choice implies a prior distribution on the $\pi$ that enjoys desirable
properties.
* $p(s_i)$ a strongly informative prior distribution, based on available data (as
illustrated above).
...
plot:      
```{r, include=TRUE, eval=FALSE, echo=TRUE, tidy=FALSE, message=TRUE, fig.dim = c(8, 6), fig.align="center"}
post.pi <- ans[['pars']][['PI']]
