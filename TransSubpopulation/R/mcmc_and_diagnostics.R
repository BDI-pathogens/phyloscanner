mcmc.sequence.approximation<- function(S.DTL.prior,TR_OBS)
{
  require(data.table)
  ...
  # set up mcmc objects
  mc<-list()
  mc$n<-(nrow(S.DTL.prior$TR_SEQ_P)-1)*(length(TR_OBS)+1)
  mc$pars<-list()
  mc$pars$LAMBDA<-matrix(NA_real_, ncol=ncol(S.DTL.prior$TR_SEQ_P), nrow=1)		#prior for proportions
  mc$pars$S<-matrix(NA_real_, ncol=ncol(S.DTL.prior$TR_SEQ_P), nrow=mc$n)	#sampling probabilities (product)
  mc$pars$S_DTL<-data.table()														#sampling probabilities (detailed terms that make up product)
  mc$pars$Z<-matrix(NA_integer_, ncol=ncol(S.DTL.prior$TR_SEQ_P), nrow=mc$n) #augmented data
  mc$pars$NU<-NA_real_															#prior for N
  mc$pars$N<-matrix(NA_integer_, ncol=1, nrow=mc$n)							#total number of counts on augmented data
  mc$pars$PI<-matrix(NA_real_, ncol=ncol(S.DTL.prior$TR_SEQ_P), nrow=mc$n)	#proportions
  mc$it.info<-data.table(	IT= seq.int(mc$n),
  ...
  mc$pars$LAMBDA[1,]	<- 0.8/ncol(S.DTL.prior$TR_SEQ_P)
  # 	sampling: set to draw from prior
  dc <- data.table(TR_SEQ_P = S.DTL.prior$TR_SEQ_P[1,],
                   REC_SEQ_P = S.DTL.prior$REC_SEQ_P[1,],
                   TR_SEQ_LOGD = S.DTL.prior$TR_SEQ_P_LOGD[1,],
                   REC_SEQ_LOGD = S.DTL.prior$REC_SEQ_P_LOGD[1,])
  dc[,COUNT_ID:=seq_len(ncol(S.DTL.prior$TR_SEQ_P))]
  
  mc$pars$S_DTL	<- copy(dc)
  ...
  mc$pars$NU			<- sum(TR_OBS) / mean(S.DTL.prior$EMP_S)
  ...
  for(i in 1L:(mc$n-1L))
  {
    ...
    if(update.count<mc$sweep)
    {
      ...
      tmp						<- dc[COUNT_ID==update.count, list(TR_SEQ_P= S.DTL.prior$TR_SEQ_P[update.round+1,update.count],
                                                  REC_SEQ_P= S.DTL.prior$REC_SEQ_P[update.round+1,update.count]),
                     by=c('COUNT_ID')]
      tmp[, TR_SEQ_LOGD:= S.DTL.prior$TR_SEQ_P_LOGD[update.round+1,update.count]]
      tmp[, REC_SEQ_LOGD:= S.DTL.prior$REC_SEQ_P_LOGD[update.round+1,update.count]]
      ...
      Z.prop[update.count]	<- TR_OBS[update.count] + rnbinom(1, TR_OBS[update.count], S.prop[update.count])
      N.prop					<- sum(Z.prop)
      #	calculate MH ratio
      log.prop.ratio			<- sum(dnbinom(mc$pars$Z[mc$curr.it,update.count]-TR_OBS[update.count], size=TR_OBS[update.count], prob=mc$pars$S[mc$curr.it,update.count], log=TRUE)) -
        sum(dnbinom(Z.prop[update.count]-TR_OBS[update.count], size=TR_OBS[update.count], prob=S.prop[update.count], log=TRUE))
      log.fc					<- sum(dbinom(TR_OBS[update.count], size=mc$pars$Z[mc$curr.it,update.count], prob=mc$pars$S[mc$curr.it,update.count], log=TRUE)) +
        dmultinom(mc$pars$Z[mc$curr.it,], prob=mc$pars$PI[mc$curr.it,], log=TRUE) +
        dpois(mc$pars$N[mc$curr.it,], lambda=mc$pars$NU, log=TRUE)
      log.fc.prop				<- sum(dbinom(TR_OBS[update.count], size=Z.prop[update.count], prob=S.prop[update.count], log=TRUE)) +
    ...
    # store log likelihood
    tmp	<- sum( dbinom(TR_OBS, size=mc$pars$Z[mc$curr.it,], prob=mc$pars$S[mc$curr.it,], log=TRUE) ) +
  ...
  save(mc, iteration, file='core_inference_SNPIZ_mcmcEachCount.rda')
}


mcmc.sampling.approximation<- function(S.DTL.prior,TR_OBS)
{
  require(data.table)
  require(Boom)	# dirichlet
  ...
  dc <- data.table(TR_SEQ_P = S.DTL.prior$TR_SEQ_P[1,],
                   REC_SEQ_P = S.DTL.prior$REC_SEQ_P[1,],
                   TR_PART_P = S.DTL.prior$TR_PART_P[1,],
                   REC_PART_P = S.DTL.prior$REC_PART_P[1,],
                   TR_SEQ_LOGD = S.DTL.prior$TR_SEQ_P_LOGD[1,],
                   REC_SEQ_LOGD = S.DTL.prior$REC_SEQ_P_LOGD[1,],
                   TR_PART_LOGD = S.DTL.prior$TR_PART_P_LOGD[1,],
                   REC_PART_LOGD = S.DTL.prior$REC_PART_P_LOGD[1,])
  dc[,COUNT_ID:=seq_len(ncol(S.DTL.prior$TR_SEQ_P))]
  mc$pars$S_DTL	<- copy(dc)
  ...
  for(i in 1L:(mc$n-1L))
  {
    ...
    if(update.count<mc$sweep)
    {
      ...
      tmp						<- dc[COUNT_ID==update.count, list(TR_SEQ_P= S.DTL.prior$TR_SEQ_P[update.round+1,update.count],
                                                  REC_SEQ_P= S.DTL.prior$REC_SEQ_P[update.round+1,update.count],
                                                  TR_PART_P= S.DTL.prior$TR_PART_P[update.round+1,update.count],
                                                  REC_PART_P= S.DTL.prior$REC_PART_P[update.round+1,update.count]),by=c('COUNT_ID')]
      tmp[, TR_SEQ_LOGD:= S.DTL.prior$TR_SEQ_P_LOGD[update.round+1,update.count]]
      tmp[, REC_SEQ_LOGD:= S.DTL.prior$REC_SEQ_P_LOGD[update.round+1,update.count]]
      tmp[, TR_PART_LOGD:= S.DTL.prior$TR_PART_P_LOGD[update.round+1,update.count]]
      tmp[, REC_PART_LOGD:= S.DTL.prior$REC_PART_P_LOGD[update.round+1,update.count]]
  ...
  save(mc, iteration, file='core_inference_SNPIZ_mcmcEachCount_GLM_EMP.rda')
}

mcmc.diagnostics.approximation<-function(mc,burnin){
  ...
  for (i in 1L:mc$sweep){
    diagnostic$acc.rate[1,i]<-mean(mc$it.info[IT>burnin & PAR_ID==i & BLOCK=='S-Z-N',ACCEPT])
  }
  
  diagnostic$acc.rate.sorted<-sort(diagnostic$acc.rate,index.return=TRUE)
  ...
  for (i in worst.effsize.PI.index){
    traceplot(mcmc(PI[,i]),main=paste('Traceplot of Pi',i),cex.lab=2.5, cex.axis=1.5, cex.main=2.5, cex.sub=2.5)
  }
  dev.off()
  
  # autocorrelation plot for PI
  png("autocorrelation_PI.png", width = 1400, height = 800)
  par(mfrow=c(2,3))
  for (i in worst.effsize.PI.index){
    acf(mcmc(PI[,i]),main='')
  ...
  for (i in worst.effsize.S.index){
    traceplot(mcmc(S[,i]),main=paste('Traceplot of S',i),cex.lab=2.5, cex.axis=1.5, cex.main=2.5, cex.sub=2.5)
  }
  dev.off()
  
  # autocorrelation plot for S
  png("autocorrelation_S.png", width = 1400, height = 800)
  par(mfrow=c(2,3))
  for (i in worst.effsize.S.index){
    acf(mcmc(S[,i]),main='')
  ...
  for (i in worst.effsize.Z.index){
    traceplot(mcmc(Z[,i]),main=paste('Traceplot of Z',i),cex.lab=2.5, cex.axis=1.5, cex.main=2.5, cex.sub=2.5)
  }
  dev.off()
  
  # autocorrelation plot for Z
  png("autocorrelation_Z.png", width = 1400, height = 800)
  par(mfrow=c(2,3))
  for (i in worst.effsize.Z.index){
    acf(mcmc(Z[,i]),main='')
  ...
  par(ps = 12, cex = 1, cex.main = 2.5)
  par(mfrow=c(1,1))
  traceplot(mcmc(N),main='Traceplot of N',cex.lab=2.5, cex.axis=1.5, cex.main=2.5, cex.sub=2.5)
  dev.off()
  
  # autocorrelation plot for N
  png("autocorrelation_N.png", width = 600, height = 500)
  par(mfrow=c(1,1))
  acf(mcmc(N),main='')
