...
One such value is obtained per window; one value is then assigned to each integeter position in the genome by taking the mean of the values from those windows spanning this position (with windows chosen to overlap, so that multiple values for each position should smooth some stochasticity in phylogenetic inference).
...
The output from \c{tools/CalculateTreeSizeInGenomeWindows.py}, or alternatively any other csv file containing genomic positions with associated normalisation constants, can be used with the \c{--normRefFileName} option.
...
\end{verbatim}
converts the per-window tree sizes to per-position tree sizes, as required as input for the \c{--normRefFileName} option of \pat.
