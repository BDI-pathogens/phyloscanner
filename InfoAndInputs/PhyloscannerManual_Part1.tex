...
This part of \p is written in Python, and so we will refer to as the Python code.
...
This option is required if \p is to analyse the trees it produces.
...
\item \c{--x-raxml}: use this option to tell \p how to run \c{RAxML}; by default, \c{`raxmlHPC-AVX -m GTRCAT -p 1'}.
...
Temporary and output files are always created in the working directory, i.e. the directory in which the \p command was run, but with this option the output files are copied to the specified directory at the end.
...
\item \c{--exact-window-start}: normally \p retrieves all reads that fully overlap a given window, i.e. starting at or anywhere before the window start, and ending at or anywhere after the window end.
...
\item \c{--recover-clipped-ends}: the default behaviour of \p is to keep only reads that fully span the window in question.
...
As such, this option should not be used as part of normal \p usage.
...
Options to only partially run \p, stopping early or skipping steps.
\begin{itemize}
