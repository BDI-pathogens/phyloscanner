...
If you do not specify anything, we will try to find the fastest \R executable available (first \c{raxmlHPC-AVX}, then \c{raxmlHPC-SSE3}, then \c{raxmlHPC}) and use the options \c{-m GTRCAT -p 1 --no-seq-check}.
...
\item \c{--read-names-2}: as \c{--read-names-1}, except the files will show the correspondence between read names and which unique sequence they correspond to.
